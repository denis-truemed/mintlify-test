...
Overview Payment partners can add a new optional boolean parameter `use_frame` to the endpoint for creating a new payment session [Create Payment Session](/reference/create)

Setting `use_iframe` to `true` will return an iframe-compatible URL in the `redirect_url` property of the 
...
```javascript
// Example to handle iFrame checkout
