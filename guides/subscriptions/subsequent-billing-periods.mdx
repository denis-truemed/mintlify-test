...
description: "For subsequent billing periods, paymentsessions can be created via the createpaymentsession endpoint by providing the paymenttoken in the call. The user does..."
---

For subsequent billing periods, `payment_session`s can be created via the `create_payment_session` endpoint by providing the `payment_token` in the call. The user doesn’t need to be present as their on-file payment method will be charged.
...
⚠️ Outside of these two, partners may never receive a webhook on the first call `create_payment_session` with `tokenize=True`
