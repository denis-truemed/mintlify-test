...
To implement subscriptions with Truemed's `payment_token` API, partner backends should create a `payment_token` and redirect the customer to Truemed’s checkout flow via the `redirect_url` included in the response to [`create_payment_session`](/reference/create). **The customer needs to be present during the flow to store their payment method for future use.**
...
- NOTE: This guide doesn’t outline the status change flow for manual/delayed capture, as that is covered elsewhere. The core requirement remains the same in either case: orders must not be fulfilled until receiving a webhook with status=captured.
